...
class PlayState extends MusicBeatSubState
{
  ...
  **/
  ...
  function initHealthBar():Void
  {
    ...
    var xPos:Float = !Preferences.extraScoreInfo ? healthBarBG.x + healthBarBG.width - 190 : healthBarBG.x;
    scoreText = new FlxText(xPos, healthBarBG.y + 30, 0, '', 20);
  ...
  function updateScoreText():Void
  {
    ...
    if (ratingPercent != 0) percent = Std.string(Math.floor((ratingPercent * 100) * 100) / 100) + '%';
      ...
      percent = 'N/A';
    // this is basicaly what floorDecimal does in psych lmao but simplified since it only is used here :D
    var combo:Int = Highscore.tallies.combo;
    var missString:String = Preferences.badsShitsCauseMiss ? 'Combo Breaks:' : 'Misses:';
    ...
    else
    {
      if (isBotPlayMode)
      {
        scoreText.text = 'Bot Play Enabled | $missString $songMisses | Combo: $combo | Rating: $ratingFC$percent)';
      }
      else
      {
  ...
  function processInputQueue():Void
  {
    ...
    while (inputPressQueue.length > 0)
    {
      ...
      else if (Constants.GHOST_TAPPING && (!playerStrumline.mayGhostTap()) && notesInDirection.length == 0 && !Preferences.ghostTapping)
      {
  ...
  function applyScore(score:Int, daRating:String, healthChange:Float, isComboBreak:Bool, ?latency:Float = 0):Void
  {
    ...
    if (songMisses < 1)
    {
      if (bads > 0 || shits > 0) ratingFC = 'Meh. (FC, ';
      else if (goods > 0) ratingFC = 'Good (GFC, ';
      else if (sicks > 0) ratingFC = 'Perfect! (SFC, ';
    }
    else if (songMisses < 10) ratingFC = 'Shit... (SDCB, ';
  }
